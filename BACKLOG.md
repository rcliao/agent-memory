...
| # | Item | Status | Notes |
|---|------|--------|-------|
| 1 | **Make the reranker affordable before re-enabling in shell** — re-entry criteria: **<2s per query** AND **a demonstrated `recall_grounded` lift** on live traffic. | **<2s met** (2026-07-13) | Fixed the dead adaptive skip (cosine-scale thresholds never fired on RRF scores ~0.02; now scale-free RRF top1/top2 ratio, `GHOST_RERANK_SKIP_RATIO`=1.5) + window 10: production-copy p50 **1.6s Search / 1.9s Context** with full quality battery at-or-above reranker-off (personal 0.861/0.917, semantic 0.889, FAMA 1.00). Re-enabled in shell configs (`GHOST_RERANK_TOP_N=10`). The `recall_grounded` half of the gate is shell-side telemetry — measure over the coming week before calling this done. |
