...
| 2 | **V2-H6: embeddings as BLOBs** — every vector query JSON-decodes ~58k float arrays from a 551MB DB. Store 384-dim float32 as a 1536-byte BLOB (additive column or new encoding flag), decode with a cast. | open | Big constant-factor win for `searchVector`'s full scan; prerequisite for (3). Schema change — needs approval + migration. |
| 3 | **ANN index** — vector retrieval is a full namespace scan per query. Options within pure-Go: IVF-flat over BLOB vectors, or LSH buckets in SQLite. | open | Only matters at ≥10k memories; live DBs qualify (42k chunks). Do (2) first — BLOBs may make the scan cheap enough to defer this. |
